...
class PuzzlePiece:
	def __init__(self, gridwidth, piece_shape):
		...
		for orientation in range(4):
			if self.get_point(orientation, False, 0, 0) == 1 \
			and self.get_point(orientation, False, 1, 0) == 0 \
			and self.get_point(orientation, False, 0, 1) == 0:
		...
		for row in range(1, gridwidth - 1):
			for column in range(1, gridwidth - 1):
				if self.get_point(0, False, row, column) != 1:
	...
	def get_nr_of_edge_bits(self):
		...
		for index in range(1, self._gridwidth - 1):
			if self.get_point(0, False, 0, index) == 1:
				edge_bits += 1
			if self.get_point(0, False, self._gridwidth - 1, index) == 1:
				edge_bits += 1
			if self.get_point(0, False, index, 0) == 1:
				edge_bits += 1
			if self.get_point(0, False, index, self._gridwidth - 1) == 1:
				edge_bits += 1
		return edge_bits

	def get_nr_of_corner_bits(self):
		corner_bits = self.get_point(0, False, 0, 0)
		corner_bits += self.get_point(0, False, 0, self._gridwidth - 1)
		corner_bits += self.get_point(0, False, self._gridwidth - 1, 0)
		corner_bits += self.get_point(0, False, self._gridwidth - 1, self._gridwidth - 1)
		return corner_bits
	...
	def get_flippable_point(self, flipped, row_number, col_number):
		if flipped:
			index = self.get_point_index(row_number, self._gridwidth - col_number - 1)
		else:
			index = self.get_point_index(row_number, col_number)
		return self._piece_shape[index]
	
	def get_point(self, orientation, flipped, row_number, col_number):
		...
		if orientation == 0:
			return self.get_flippable_point(flipped, row_number, col_number)
		elif orientation == 1:
			return self.get_flippable_point(flipped, self._gridwidth - col_number - 1, row_number)
		elif orientation == 2:
			return self.get_flippable_point(flipped, self._gridwidth - row_number - 1, self._gridwidth - col_number - 1)
		elif orientation == 3:
			return self.get_flippable_point(flipped, col_number, self._gridwidth - row_number - 1)
		else:
			raise ValueError("Invalid orientation %s" % orientation); 

	def get_point_str(self, orientation, flipped, row_number, col_number):
		if self.get_point(orientation, flipped, row_number, col_number) == 1:
			return "X"
		else:
			return " "
...
class PuzzleLayout:
	...
	def is_valid_edge(self, location1, orientation_adj1, location2, orientation_adj2):
		...
		for index in range(1, gridwidth - 1):
			edge_value = piece1.get_point(orientation1, flipped1, gridwidth - 1, index)
			edge_value += piece2.get_point(orientation2, flipped2, 0, index)
		...
		corner_left = piece1.get_point(orientation1, flipped1, gridwidth - 1, 0) 
		corner_left += piece2.get_point(orientation2, flipped2, 0, 0)
		...
		corner_right = piece1.get_point(orientation1, flipped1, gridwidth - 1, gridwidth - 1) 
		corner_right += piece2.get_point(orientation2, flipped2, 0, gridwidth - 1)
	...
	def is_valid_corner(self, location1, orient_adj1, location2, orient_adj2, location3, orient_adj3):
		...
		#print()
		corner_bit1 = piece1.get_point(orientation1, flipped1, gridwidth - 1, gridwidth - 1)
		corner_bit2 = piece2.get_point(orientation2, flipped2, 0, gridwidth - 1)
		corner_bit3 = piece3.get_point(orientation3, flipped3, 0, 0)
